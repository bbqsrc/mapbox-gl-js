...
export function globePoleMatrixForTile(id: CanonicalTileID, isTopCap: boolean, tr: Transform) {
...
export class GlobeSharedBuffers {
    ...
    _createPoleVertices(zoom: number, isTopCap: boolean, outArr: GlobeVertexArray) {
        ...
        const tileSize = 512;
        const tiles = 1 << zoom;
        const worldSize = tiles * tileSize;
    ...
    _createPoleVerticesForAllZooms(isTopCap: boolean): GlobeVertexArray {
        const maxPoleZoom = 22;
        const vertices = new GlobeVertexArray();

        for (let zoom = 0; zoom <= maxPoleZoom; zoom++) {
